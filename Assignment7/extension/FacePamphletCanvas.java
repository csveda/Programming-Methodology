...
public class FacePamphletCanvas extends GCanvas 
	...
	public void displayProfile(FacePamphletProfile profile) {
		removeAll();
		addName(profile.getName(), profile.getSurname());
		addLocation(profile.getCity(), profile.getCountry());
		addImage(profile.getImage());
		addStatus(profile.getStatus());
		addFriends(profile.getFriends());
	}
	
	private void addName(String name, String surname) {
		String str = name + " " + surname;
		nameLabel = new GLabel(str);
	...
	private void addLocation(String city, String country) {
		String str = "From " + city + "," + country;
		GLabel label = new GLabel(str);
		label.setFont(PROFILE_NAME_FONT);
		label.setColor(Color.BLUE);
		
		double x = LEFT_MARGIN;
		double y = TOP_MARGIN + IMAGE_MARGIN + IMAGE_HEIGHT + STATUS_MARGIN + 100;
		add(label, x, y);
	}
	...
	private void addFriends(Iterator <String> friends) {
		friendsLabel = new GLabel("Following : ");
